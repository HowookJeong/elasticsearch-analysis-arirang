...
public class ArirangAnalyzerRestAction extends BaseRestHandler {
  ...
  protected RestChannelConsumer doArirangJamoTokenizer(RestRequest restRequest, NodeClient client) throws IOException {
    String jamoToken = "";

    try {
      jamoToken = HanguelJamoMorphTokenizer.getInstance().tokenizer(
        restRequest.param("text", ""),
        restRequest.param("token", HanguelJamoType.CHOSUNG.getName())
      );
    } catch (Exception e) {
      return channel -> channel.sendResponse(new BytesRestResponse(RestStatus.NOT_ACCEPTABLE, "Failed which jamo tokenizer!!"));
    } finally {
    }
  ...
  protected RestChannelConsumer doArirangDictionaryReload(RestRequest restRequest, NodeClient client) throws IOException {
    try {
